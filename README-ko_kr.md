...
> **저장 공간에 대해.** 모델 다운로드 자체는 약 4GB입니다. 다만 Chrome은 다운로드를 시작하기 전에 Chrome 프로필이 있는 볼륨에 **22GB 이상 여유**를 요구하고, 받은 뒤 여유가 **10GB 밑으로 떨어지면 모델을 삭제합니다.** 크기는 모델 버전마다 다르며 현재 값은 `chrome://on-device-internals`에서 확인할 수 있습니다.
>
> 그래서 가용성은 한 번 확인하고 끝낼 값이 아닙니다. 어제 되던 기기가 디스크가 차면서 오늘 `unavailable`을 돌려줄 수 있고, 그 사실이 에러로 드러나지도 않습니다.
...
- **모두에게 동작해야 하는 핵심 기능** — Chrome 데스크톱만 지원하고, 그중에서도 여유 공간 22GB 관문을 통과하고 VRAM 4GB를 넘기는(또는 RAM 16GB + 4코어 이상으로 CPU 경로를 타는) 기기에서만 모델이 켜집니다. 부가 기능으로 쓰거나 서버 폴백을 두세요.
